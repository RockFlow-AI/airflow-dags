...
DAG_ID = "symbol_download_otc"

OTC_RAW_KEY = f'{DAG_ID}_otc'

SYMBOL_PARSE_KEY = f'{DAG_ID}_parse/'
SYMBOL_SOURCE_KEY = f'{SYMBOL_PARSE_KEY}apollootc.csv'

with DAG(
        DAG_ID,
        catchup=False,
        start_date=pendulum.datetime(2022, 5, 18, tz='America/New_York'),
        schedule_interval='50 0 * * 1-5',
        default_args={
            "owner": "daijunkai",
    ...
    symbol_raw = OtcSymbolDownloadOperator(
        key=OTC_RAW_KEY
    )

    symbol_source = OtcSymbolParser(
