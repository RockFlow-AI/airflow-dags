...
SimpleHttpOperator(
    ...
    method='PATCH',
    http_conn_id='flow-statement',
    endpoint='/inner/clearing/handle?file=Cash_InOut&date={date}'.format(date=(datetime.now()).strftime("%Y%m%d")),
    response_check=lambda response: response.json()['code'] == 200,
    extra_options={"timeout": 60},
...
SimpleHttpOperator(
    ...
    method='PATCH',
    http_conn_id='flow-statement',
    endpoint='/inner/clearing/handle?file=Position_InOut&date={date}'.format(date=(datetime.now()).strftime("%Y%m%d")),
    response_check=lambda response: response.json()['code'] == 200,
    extra_options={"timeout": 60},
